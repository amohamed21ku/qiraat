// Tasks/Stage1_Editor_TaskPage.dart - Managing Editor Tasks for Stage 1 Approval Workflow
...
class Stage1EditorTasksPage extends StatefulWidget {
  @override
  _Stage1EditorTasksPageState createState() => _Stage1EditorTasksPageState();
}

class _Stage1EditorTasksPageState extends State<Stage1EditorTasksPage>
  ...
  // Editor-specific statuses for Stage 1
  final List<String> editorStatuses = [
    AppConstants.SECRETARY_APPROVED, // Ready for editor review
    AppConstants.SECRETARY_EDIT_REQUESTED, // After edit request from secretary
    AppConstants.EDITOR_REVIEW, // Currently under editor review
    AppConstants.EDITOR_APPROVED, // Editor approved
    AppConstants.EDITOR_REJECTED, // Editor rejected
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: editorStatuses.length, vsync: this);
  ...
  Future<void> _loadTaskCounts() async {
    ...
    try {
      for (String status in editorStatuses) {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      Text(
                        'المرحلة الأولى: مراجعة المحتوى والملاءمة',
                        style: TextStyle(
                          fontSize: 16,
  ...
  Widget _buildStatisticsCards() {
    ...
    final pendingReview = (statusCounts[AppConstants.SECRETARY_APPROVED] ?? 0) +
        (statusCounts[AppConstants.SECRETARY_EDIT_REQUESTED] ?? 0) +
        (statusCounts[AppConstants.EDITOR_REVIEW] ?? 0);
    final completed = (statusCounts[AppConstants.EDITOR_APPROVED] ?? 0) +
    ...
    final awaitingHeadEditor = completed;
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(
        ...
        tabs: editorStatuses.map((status) {
  ...
  Widget _buildTaskCard(DocumentSnapshot document, String status) {
    ...
    final secretaryDecision = _getSecretaryDecision(data);
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Secretary Decision (if available)
                if (secretaryDecision.isNotEmpty) ...[
                  SizedBox(height: 12),
                  Container(
                    padding: EdgeInsets.all(12),
                    decoration: BoxDecoration(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'قرار السكرتير: $secretaryDecision',
                            style: TextStyle(
                              fontSize: 12,
                ...
                if (_getNextAction(status).isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.next_plan, color: Colors.green, size: 16),
                        SizedBox(width: 8),
                        Expanded(
                          child: Text(
  ...
  String _getSecretaryDecision(Map<String, dynamic> data) {
    if (data.containsKey('secretaryDecision')) {
      String decision = data['secretaryDecision'];
      switch (decision) {
        case 'approved':
          return 'موافقة';
        case 'edit_requested':
  ...
  String _getNextAction(String status) {
    switch (status) {
      ...
      case AppConstants.SECRETARY_EDIT_REQUESTED:
        return 'بدء المراجعة';
      case AppConstants.EDITOR_REVIEW:
        return 'اتخاذ قرار (موافقة/رفض/موقع/تعديل)';
      case AppConstants.EDITOR_APPROVED:
      case AppConstants.EDITOR_REJECTED:
      case AppConstants.EDITOR_WEBSITE_RECOMMENDED:
      case AppConstants.EDITOR_EDIT_REQUESTED:
        return 'في انتظار رئيس التحرير';
      default:
        return '';
    }
  }
