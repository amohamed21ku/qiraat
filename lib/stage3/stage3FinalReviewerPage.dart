// pages/Stage3/Stage3FinalReviewerPage.dart
...
class Stage3FinalReviewerPage extends StatefulWidget {
  final DocumentModel document;

  const Stage3FinalReviewerPage({Key? key, required this.document})
      : super(key: key);

  @override
  _Stage3FinalReviewerPageState createState() =>
      _Stage3FinalReviewerPageState();
}

class _Stage3FinalReviewerPageState extends State<Stage3FinalReviewerPage>
  ...
  // Review checkboxes
  bool _contentAccuracy = false;
  bool _languageCorrectness = false;
  bool _formatConsistency = false;
  bool _referencesCheck = false;
  bool _layoutQuality = false;
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Icon(Icons.fact_check, color: Colors.white, size: 28),
              ),
              SizedBox(width: 16),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'التدقيق والمراجعة النهائية قبل النشر',
                      style: TextStyle(
                        fontSize: 16,
  ...
  Widget _buildStatusBar() {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.fact_check, color: Colors.white, size: 24),
          SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Text(
                  'جاهز للمراجعة النهائية قبل النشر',
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
  ...
  Widget _buildLayoutDesignInfoCard() {
    // Show the layout design work from the layout designer
    return Container(
      margin: EdgeInsets.only(bottom: 20),
      padding: EdgeInsets.all(20),
      decoration: BoxDecoration(
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'ملف الإخراج النهائي من المخرج الفني',
                  style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
                    color: Colors.purple.shade700,
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        'يرجى مراجعة الإخراج الفني والتأكد من جودته قبل الموافقة النهائية.',
                        style: TextStyle(
                          fontSize: 12,
  ...
  Widget _buildReviewForm() {
    bool canSubmit = _document!.status == AppConstants.FINAL_REVIEW_STAGE;
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 12),
                  _buildChecklistItem(
                    'دقة المحتوى والمعلومات',
                    _contentAccuracy,
                    (value) =>
                        setState(() => _contentAccuracy = value ?? false),
                  ),
                  _buildChecklistItem(
                    'صحة اللغة والأسلوب',
                    _languageCorrectness,
                    (value) =>
                        setState(() => _languageCorrectness = value ?? false),
                  ),
                  _buildChecklistItem(
                    'اتساق التنسيق والإخراج',
                    _formatConsistency,
                    (value) =>
                        setState(() => _formatConsistency = value ?? false),
                  ),
                  _buildChecklistItem(
                    'صحة المراجع والاستشهادات',
                    _referencesCheck,
                    (value) =>
                        setState(() => _referencesCheck = value ?? false),
                  ),
                  _buildChecklistItem(
                    'جودة الإخراج الفني',
                    _layoutQuality,
                    (value) => setState(() => _layoutQuality = value ?? false),
                  ),
                  SizedBox(height: 20),

                  // Comments
                  Text(
                    'ملاحظات المراجعة النهائية (مطلوب):',
                    style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: Colors.grey.shade800,
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'اكتب ملاحظاتك النهائية على المقال...',
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'أية توصيات أو تعديلات مطلوبة على الإخراج...',
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(8),
                      ),
                    ),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'أية ملاحظات أو توضيحات إضافية...',
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(8),
                      ),
                    ),
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text(
                              'إرفاق تقرير المراجعة النهائية (اختياري)',
                              style: TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.bold,
                                color: Colors.grey.shade800,
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: (_canSubmitReview() && !_isUploading)
                          ? _submitReview
                          : null,
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'تم إنهاء المراجعة النهائية لهذا المقال.',
                        style: TextStyle(
                          fontSize: 14,
                          color: Colors.blue.shade700,
                          fontWeight: FontWeight.w600,
  ...
  Future<void> _submitReview() async {
    ...
    try {
      final reviewData = {
        ...
        'checklist': {
          'contentAccuracy': _contentAccuracy,
          'languageCorrectness': _languageCorrectness,
          'formatConsistency': _formatConsistency,
          'referencesCheck': _referencesCheck,
          'layoutQuality': _layoutQuality,
        },
      };

      await _documentService.submitFinalReview(
        _document!.id,
        _currentUserId!,
        _currentUserName!,
        reviewData,
      ...
      _showSuccessSnackBar('تم إرسال المراجعة النهائية بنجاح');
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        final storageRef = _storage.ref().child(
            'final_reviews/${_document!.id}/${_currentUserId}_${DateTime.now().millisecondsSinceEpoch}_${file.name}');
          ...
          uploadTask = storageRef.putData(
            ...
            SettableMetadata(
              ...
              customMetadata: {
                ...
                'uploadType': 'final_review_report',
              },
            ),
          );
        } else {
